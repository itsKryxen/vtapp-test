...
export default function MinimalTechCard({ event, clubName, index = 0 }: EventCardProps) {
  ...
  return (
    <div 
      className="group relative flex flex-col h-full bg-white/5 border border-white/10 hover:border-white/25 transition-all duration-300 overflow-hidden"
    ...
    >
      ...
      {/* Top Header */}
      <div className="flex items-center px-4 py-2 border-b border-white/10 relative z-10 bg-ink-950/80">
      ...
      {/* Action Footer */}
      <div className="grid grid-cols-2 border-t border-white/10 relative z-10 bg-ink-950/80">
        <Link 
          href={`/events/${event.slug}`}
          className="flex items-center justify-center py-3 font-mono text-[10px] uppercase tracking-widest text-slate-300 hover:bg-[var(--card-accent)] hover:text-white transition-colors duration-200 border-r border-white/10 group/explore"
        >
